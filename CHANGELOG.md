...
- **`App.xaml.cs` is five files instead of one.** It had reached 1,793 lines holding the tray menu, the update path, every dialog the tray opens and the whole settings surface in one type. Those four clusters moved to `App.Tray.cs`, `App.Updates.cs`, `App.Dialogs.cs` and `App.Settings.cs` as partials of the same class, leaving 575 lines of startup, shutdown, crash handling and the backup commands. Members moved verbatim with their doc comments; the significant lines before and after are the same multiset, so nothing was reworded on the way.
