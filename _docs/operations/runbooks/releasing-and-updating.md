...
- **Daily**, if *Check for updates on its own* is on (it is, by default). The check runs at
  startup and again every 24 hours while the app is running, driven by the tick that already runs
  every 15 seconds. It used to run only when the Settings dialog was opened, which meant a user
  who never opened Settings was never told a fix existed.
...
The last-checked time is recorded **even when the check failed**. Without that, a machine that is
offline, behind a proxy or rate-limited never records an attempt and re-checks on every tick.

Whichever path ran the check, the result is recorded in one place, so the three surfaces below
cannot disagree with each other about what is available.

### Saying so

An available update is said in three places, and nothing appears when you are up to date
([[ADR-018]]):

- **The status strip**, as a fourth segment: `· UPDATE 0.7.6 AVAILABLE`.
- **The tray menu**, as a line above `LAST BACKUP` that opens Settings. It ships collapsed, so an
  up-to-date menu is exactly the designed one.
- **A notification**, once per version. Not once per check and not once per launch: an update
  stays available until it is installed, so per-episode would mean once ever and per-process would
  nag on every launch.

This is a third notification where the design allows two. The rule that budget protects is
untouched: a successful backup still never notifies, and `TrayNotifications` takes no completed
backup as an input.

### Installing

Only ever from a press. *"It never installs anything without you"* is structural rather than a
promise: `UpdateViewModel` takes no success as an input, and the only unprompted acts in the whole
feature are the daily look and the notice that follows it.
...
5. **Swap**, from the staged copy, which is outside the directory being replaced. A process cannot
   overwrite its own executable while running; that is why there are two. Each rename is retried
   for two and a half seconds: the old process exiting is not the same as Windows finishing with
   its files, and an image section, a shell extension or a scanner reading the freshly-extracted
   DLLs will each hold the folder for a moment.
6. **Relaunch**, either way, the new install on success, the old one on failure.
7. **Record a failure**, if there was one, beside `settings.json` rather than in the install
   directory, which is the thing being renamed. The swap runs after the window the user was
   looking at has gone, so there is nothing left to report into; the next launch reads the note
   once and says so on the status strip and as a notification.
...
| App relaunches on the OLD version after an update | The swap failed and rolled back, which is the designed outcome. The app says `UPDATE DIDN'T INSTALL` on the next launch, and `<install>.staged` will still be there as corroboration. |
...
| Every update fails its checksum | Fixed in 0.7.6. Before that, the feed took any asset ending `.sha256` as the checksum for whatever it was downloading, so from 0.7.2 (when the CLI became its own artifact) the app verified its archive against the CLI's digest. Builds before 0.7.6 cannot update themselves and have to be replaced by hand once. See [[every-update-fails-its-checksum]]. |
